...
# (data key, SOAP field, name suffix, cooling-only)
_SETPOINTS = [
    ("night_setback_heating", RC_FIELD_NIGHT_HEATING, "Nachtabsenkung", False),
    ("absence_setback_heating", RC_FIELD_ABSENCE_HEATING, "Abwesenheit", False),
    ("set_temp_cooling", RC_FIELD_SET_COOLING, "Sollwert Kühlen", True),
    ("night_setback_cooling", RC_FIELD_NIGHT_COOLING, "Nachtabsenkung Kühlen", True),
    ("absence_setback_cooling", RC_FIELD_ABSENCE_COOLING, "Abwesenheit Kühlen", True),
]
...
async def async_setup_entry(
    ...
    entities = [
        InnoxelRoomClimateSetpoint(slow, client, entry.entry_id, idx, name, spec)
        for idx, name in sorted(coordinator.room_climate_modules.items())
        for spec in _SETPOINTS
        if cooling or not spec[3]
    ]
...
class InnoxelRoomClimateSetpoint(CoordinatorEntity, NumberEntity):
    ...
    def __init__(self, coordinator, client, entry_id, idx, room_name, spec):
        ...
        data_key, soap_field, name_suffix, _ = spec
